...
const outDir = "../../npm/packages/chacha20poly1305";
...
await build({
  ...
  postBuild() {
    ...
    try {
      ...
      Deno.copyFileSync(
        "test/vectors/chacha20_poly1305_test.json",
        `${outDir}/esm/test/vectors/chacha20_poly1305_test.json`,
      );
      Deno.mkdirSync(`${outDir}/script/test/vectors`, { recursive: true });
      Deno.copyFileSync(
        "test/vectors/chacha20_poly1305_test.json",
        `${outDir}/script/test/vectors/chacha20_poly1305_test.json`,
      );
    } catch {
