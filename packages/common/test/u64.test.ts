...
// const big = (n) => {
//   return {
//     h: Number((n >> 32n) & 0xffff_ffffn) | 0,
//     l: Number(n & 0xffff_ffffn) | 0,
//   };
// };
...
describe("u64", () => {
  it("shr_small", () => {
    ...
    for (let i = 0, bi = 0n; i < 32; i++, bi += 1n) {
      const h = u64.shrSH(val[0], val[1], i);
      const l = u64.shrSL(val[0], val[1], i);
      assertEquals((big >> bi) & U64_MASK, u64.toBig(h, l));
    }
  });
  ...
  //     deepStrictEqual((big >> iBig) & U64_MASK, u64.toBig(h, l));
  //   }
  // });
  ...
  it("rotr_small", () => {
    const val = [0x01234567, 0x89abcdef];
    const big = u64.toBig(val[0], val[1]);
    for (let i = 1, bi = 1n; i < 32; i++, bi += 1n) {
      const h = u64.rotrSH(val[0], val[1], i);
      const l = u64.rotrSL(val[0], val[1], i);
      assertEquals(rotate_right(big, bi), u64.toBig(h, l));
    }
  });
  ...
  it("rotr32", () => {
    ...
    assertEquals(rotate_right(big, 32n), u64.toBig(h, l));
  });

  it("rotr_big", () => {
    const val = [0x01234567, 0x89abcdef];
    const big = u64.toBig(val[0], val[1]);
    for (let i = 33, bi = 33n; i < 64; i++, bi += 1n) {
