/**
 * This file is imported from noble-hashes (https://github.com/paulmillr/noble-hashes).
 *
 * noble-hashes - MIT License (c) 2022 Paul Miller (paulmillr.com)
 *
 * The original file is located at:
 * https://github.com/paulmillr/noble-hashes/blob/4e358a46d682adfb005ae6314ec999f2513086b9/test/hmac.test.ts
 */
...
describe("hmac", () => {
  for (let i = 0; i < HMAC_VECTORS.length; i++) {
    ...
    describe("vector " + i, () => {
      it("sha256 full", () => {
        ...
        assertEquals(
          truncate(h256.digest(), t.truncate as number),
          hexToBytes(t.sha256),
        );
      });
      it("sha256 partial", () => {
        const h256 = hmac.create(sha256, t.key);
        for (const d of t.data) h256.update(d);
        assertEquals(
          truncate(h256.digest(), t.truncate as number),
          hexToBytes(t.sha256),
        );
      ...
      it("sha512 full", () => {
        ...
        assertEquals(
          truncate(h512.digest(), t.truncate as number),
          hexToBytes(t.sha512),
        );
      });
      it("sha512 partial", () => {
        const h512 = hmac.create(sha512, t.key);
        for (const d of t.data) h512.update(d);
        assertEquals(
          truncate(h512.digest(), t.truncate as number),
          hexToBytes(t.sha512),
        );
  ...
  it("HMAC types", () => {
    ...
    for (const t of TYPE_TEST.bytes) {
      assertThrows(() => hmac(sha256, t, msg), Error);
      assertThrows(() => hmac(sha256, key, t), Error);
      assertThrows(
        () => hmac.create(sha256, t),
        Error,
      );
    }
    assertThrows(
      () => hmac(sha256, undefined as unknown as Uint8Array, msg),
      Error,
    );
    assertThrows(
      () => hmac(sha256, key, undefined as unknown as Uint8Array),
      Error,
    );
    assertThrows(
      () => hmac.create(sha256, undefined as unknown as Uint8Array),
      Error,
    );
    ...
    for (const t of TYPE_TEST.hash) {
      assertThrows(() => hmac(t, key, msg), Error);
    }
    assertEquals(
  ...
  it("not be created with invalid hash fn", () => {
    ...
    assertThrows(
      () => hmac(fakeHash as unknown as CHash, EMPTY.bytes, EMPTY.bytes),
      Error,
    );
  });
});
